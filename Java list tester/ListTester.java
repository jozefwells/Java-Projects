...
public class ListTester {
	...
	private void runTests() {
		...
		testSingleElementList(A_set0B_B, "A_setB_B", LIST_B, STRING_B);
		testSingleElementList(A_iterSetAfterNextB_B, "A_iterSetAfterNextB_B", LIST_B, STRING_B);
...
//			break;
	...
	/** Scenario: [A,B,C] -> iterRemoveAfterNextNextB -> [A,C]
	 * @return [A,C] after iterRemoveAfterNextNextB
	 */
   ...
   /** Scenario: [A,B] -> iterRemoveAfterNextB -> [A]
	...
	/** Scenario: [] -> iterAdd(A) -> [A]
	 * @return [A] after iterAdd(A)
	 */
	...
	/** Scenario: [A] -> iterNext()iterSet(B) -> [B]
	 * @return [A] after iterNext()iterSet(B)
	 */
	private IndexedUnsortedList<Integer> A_iterSetAfterNextB_B() {
		IndexedUnsortedList<Integer> list = emptyList_iterAddA_A(); 
		ListIterator<Integer> it = list.listIterator();
		it.next();
		it.set(ELEMENT_B);
		return list;
	...
	/** Scenario: [A,B] -> iterAdd(C) -> [C,A,B]
	 * @return [C,A,B] after iterAdd(C)
	 */
	...
	/** Scenario: [A,B,C] -> iterNext()iterRemove(B) -> [A,C]
	 * @return [A,C] after iterNext()iterRemove(B)
	 */
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_testRemove" + contentsString.charAt(0), testRemoveElement(scenario.build(), contents[0], Result.MatchingValue));
			printTest(scenarioName + "_testRemoveNext" + contentsString.charAt(1), testRemoveElement(scenario.build(), contents[1], Result.MatchingValue));
			printTest(scenarioName + "_testRemoveX", testRemoveElement(scenario.build(), ELEMENT_X, Result.NoSuchElement));
			printTest(scenarioName + "_testFirst", testFirst(scenario.build(), contents[0], Result.MatchingValue));
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_testIndexOf0" + contentsString.charAt(0), testIndexOf(scenario.build(), contents[0], 0));
			printTest(scenarioName + "_testIndexOf1" + contentsString.charAt(1), testIndexOf(scenario.build(), contents[1], 1));
			printTest(scenarioName + "_testIndexOf1" + contentsString.charAt(2), testIndexOf(scenario.build(), contents[2], 2));
			printTest(scenarioName + "_testIndexOfX", testIndexOf(scenario.build(), ELEMENT_X, -1));
			printTest(scenarioName + "_testRemoveNeg1", testRemoveIndex(scenario.build(), -1, null, Result.IndexOutOfBounds));
			printTest(scenarioName + "_testRemove0", testRemoveIndex(scenario.build(), 0, contents[0], Result.MatchingValue));
			printTest(scenarioName + "_testRemove1", testRemoveIndex(scenario.build(), 1, contents[1], Result.MatchingValue));	
			...
			printTest(scenarioName + "_iterNextNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(scenarioName + "_iterNexNexttNext_testIterHasNext", testIterHasNext(iterAfterNext(scenario.build(), 3), Result.False));
